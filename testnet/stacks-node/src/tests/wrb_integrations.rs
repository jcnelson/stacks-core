...
use crate::tests::nakamoto_integrations::{
    blind_signer, boot_to_epoch_3, naka_neon_integration_conf, next_block_and_mine_commit,
    setup_stacker, wait_for,
};
use crate::tests::neon_integrations::{
    call_read_only, get_chain_info_result, submit_tx, test_observer, wait_for_runloop,
};
...
pub fn wrb_playground() {
    ...
    let sender_sk = Secp256k1PrivateKey::from_hex(
        "e89bb394ecd5161007a84b34ac98d4f7239016c91d3e0c7c3b97aa499693288301",
    )
    .unwrap();
    let home_sk = Secp256k1PrivateKey::from_hex(
        "05dbe6a6760e62ebda8d6817f857751f9daf7ab17d91d775613eff7d09a4eaaf01",
    )
    .unwrap();
    let app_sk = Secp256k1PrivateKey::from_hex(
        "1c9e5672eb18d703b984f9fb9ff74a8e2d889fb0165014ab8ec74701701ba20a01",
    )
    .unwrap();
    ...
    naka_conf.add_initial_balance(PrincipalData::from(sender_addr).to_string(), 10000000);
    naka_conf.add_initial_balance(PrincipalData::from(home_addr).to_string(), 10000000);
    naka_conf.add_initial_balance(PrincipalData::from(app_addr).to_string(), 1000000000000);
    naka_conf.add_initial_balance(PrincipalData::from(sender_signer_addr).to_string(), 100000);

    // subscribe to wrbpods
    naka_conf.node.stacker_dbs.append(&mut vec![
    ...
    // expand call-readonly
    naka_conf.connection_options.read_only_call_limit = ExecutionCost {
    ...
    // get the name price
    let price_value = call_read_only(
        &naka_conf,
        &sender_addr,
        "BNS-V2",
        "get-name-price",
    ...
    eprintln!("Price of {}.{} is {:?}", name, namespace, &price_value);
    ...
    let preorder_tx = make_contract_call(
        ...
        &vec![
            Value::buff_from(
                Hash160::from_data(format!("{}.{}.salt", name, namespace).as_bytes())
                    .0
                    .to_vec(),
            )
            .unwrap(),
